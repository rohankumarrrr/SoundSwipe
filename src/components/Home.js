...
const REDIRECT_URI = "https://rohankumarrrr.github.io/SoundSwipe/#/callback"; // Replace with your redirect URI
...
const Home = () => {
  ...
  useEffect(() => {
    // Extract token and other params from the URL hash
    const { access_token, expires_in, token_type, error } = queryString.parse(window.location.hash);
  ...
  return (
    <div className="App">
      <header className="App-header-div">
        <p className="App-header" style={{ fontWeight: "bold", fontStyle: "normal" }}>Sound</p>
        <p className="App-header">Swipe</p>
      </header>
      {!token ? (
